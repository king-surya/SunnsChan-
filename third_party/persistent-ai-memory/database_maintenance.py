...
from utils import get_memory_data_dir
...
class DatabaseMaintenance:
    ...
    def __init__(self, memory_system, memory_data_path: str = None):
        ...
        archives_dir = self.memory_data_path / "archives"
        backups_dir = self.memory_data_path / "backups"
    ...
    async def check_and_rotate_if_needed(self, db_type: str) -> Dict[str, any]:
        ...
        This is the main orchestration method called by AIMemorySystem.
    ...
    async def _build_tag_registries(self) -> Dict:
        ...
        """
        tag_registry_path = os.path.join(get_memory_data_dir(), "tag_registry.json")
        results = {
            "main_registry_path": str(tag_registry_path),
            "docker_registry_path": str(tag_registry_path),  # Same location for consistency
            "tags_found": 0,
            "memories_scanned": 0,
            "errors": []
        }
        ...
        try:
            ...
            if memories_to_process:
                ...
                # Save to both locations (now the same for consistency)
                main_path = Path(tag_registry_path)
                docker_path = Path(tag_registry_path)
    ...
    async def _build_memory_bank_registries(self) -> Dict:
        ...
        """
        memory_bank_registry_path = os.path.join(get_memory_data_dir(), "memory_bank_registry.json")
        results = {
            "main_registry_path": str(memory_bank_registry_path),
            "docker_registry_path": str(memory_bank_registry_path),  # Same location for consistency
            "banks_found": 0,
            "total_memories": 0,
            "errors": []
        }
        ...
        try:
            ...
            # Save to both locations (now the same for consistency)
            main_path = Path(memory_bank_registry_path)
            docker_path = Path(memory_bank_registry_path)
